...
public class EnemyAim : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GetComponentInParent<EnemyBasic>().isFacingRight && transform.rotation.z < -z)
        {
            rot.z = -z;
            transform.rotation = rot;
        }
        else if (!GetComponentInParent<EnemyBasic>().isFacingRight && transform.rotation.z > z)
        {
            rot.z = z;
            transform.rotation = rot;
        }

        if (diffrence.x > 0 && !GetComponentInParent<EnemyBasic>().isFacingRight)
        {
            firePos.rotation = Quaternion.Euler(0, 0, 0);
            GetComponentInParent<EnemyBasic>().flip();
        }
        else if (diffrence.x < 0 && GetComponentInParent<EnemyBasic>().isFacingRight)
        {
